...
#define QUALITY_CVAR_LOWPRIORITY_AT_LEAST(NAME,BOOSTVAL) SetCapturedCVarPredicated(NAME, BOOSTVAL, std::greater<float>(), !bHighQualityModeDesired, false)
		...
		)
	{
		...
		QUALITY_CVAR("r.gbufferformat", 5); // 5 = highest precision OK
		...
		// LOD Settings
		QUALITY_CVAR("r.TextureStreaming",0); //OK
		QUALITY_CVAR("r.ForceLOD",0);			//OK
		QUALITY_CVAR("r.particlelodbias", -10);	//OK
		QUALITY_CVAR("foliage.DitheredLOD", 0);	//OK
		QUALITY_CVAR("foliage.ForceLOD", 0);	//OK
		//QUALITY_CVAR("r.HLOD", 0);				//OK
		
		// 其他设定
		QUALITY_CVAR("r.D3D12.GPUTimeout", 0); //OK
		QUALITY_CVAR("a.URO.Enable", 0);	//ok
		
		// 天光设定
		QUALITY_CVAR("r.SkyLight.RealTimeReflectionCapture.TimeSlice",0);	//ok
		QUALITY_CVAR("r.VolumetricRenderTarget",0);							//ok
		
		// ~sg.AntiAliasingQuality @ cine 
		//抗锯齿选项
		QUALITY_CVAR_AT_LEAST("r.AntiAliasingMethod", 2); //这里是抗锯齿方式这里是超分辨率 原为TAA：2	//ok
		QUALITY_CVAR_AT_LEAST("r.TemporalAASamples",64);										//ok
		QUALITY_CVAR_AT_LEAST("r.TemporalAAFilterSize",1);										//ok
		//QUALITY_CVAR_AT_LEAST("r.ngx.dlss.quality", 2); // high-quality mode for DLSS if in use //NOT
		
		// ~sg.EffectsQuality @ cinematic
		QUALITY_CVAR_AT_LEAST("r.TranslucencyLightingVolumeDim", 64);							//ok
		QUALITY_CVAR("r.RefractionQuality", 2);													//ok
		QUALITY_CVAR("r.SSR.Quality", 4);														//ok
		// QUALITY_CVAR("r.SceneColorFormat", 4); // no - don't really want to mess with this
		QUALITY_CVAR("r.TranslucencyVolumeBlur", 1);											//ok
		
		QUALITY_CVAR("r.MaterialQualityLevel", 1); // 0==low, -> 1==high <- , 2==medium			//OK
		QUALITY_CVAR("r.SSS.Scale", 1);
		QUALITY_CVAR("r.SSS.SampleSet", 2);	//oK
		QUALITY_CVAR("r.SSS.Quality", 1);		//OK
		QUALITY_CVAR("r.SSS.HalfRes", 0);		//OK
		...
		QUALITY_CVAR("r.AmbientOcclusionMipLevelFactor", 0.4f);	//OK
		QUALITY_CVAR("r.AmbientOcclusionMaxQuality", 100);		//OK
		QUALITY_CVAR("r.AmbientOcclusionLevels", -1);			//OK
		QUALITY_CVAR("r.AmbientOcclusionRadiusScale", 1.f);		//OK
		QUALITY_CVAR("r.DepthOfFieldQuality", 4);				//OK
		QUALITY_CVAR_AT_LEAST("r.RenderTargetPoolMin", 500); // ?ok
		QUALITY_CVAR("r.LensFlareQuality", 3);	//ok
		QUALITY_CVAR("r.SceneColorFringeQuality", 1);	//OK
		QUALITY_CVAR("r.BloomQuality", 5);	//OK
		QUALITY_CVAR("r.FastBlurThreshold", 100);	//OK
		QUALITY_CVAR("r.Upscale.Quality", 3);	//OK
		QUALITY_CVAR("r.Tonemapper.GrainQuantization", 1);	//OK
		QUALITY_CVAR("r.LightShaftQuality", 1);		//OK
		QUALITY_CVAR("r.Filter.SizeScale", 1);	//OK
		QUALITY_CVAR("r.Tonemapper.Quality", 5);	//OK
		QUALITY_CVAR("r.DOF.Gather.AccumulatorQuality", 1);	//OK
		QUALITY_CVAR("r.DOF.Gather.PostfilterMethod", 1);	//OK
		QUALITY_CVAR("r.DOF.Gather.EnableBokehSettings", 1);	//OK
		QUALITY_CVAR_AT_LEAST("r.DOF.Gather.RingCount", 5);		//OK
		QUALITY_CVAR("r.DOF.Scatter.ForegroundCompositing", 1);	//OK
		QUALITY_CVAR("r.DOF.Scatter.BackgroundCompositing", 2);	//OK
		QUALITY_CVAR("r.DOF.Scatter.EnableBokehSettings", 1);	//OK
		QUALITY_CVAR("r.DOF.Scatter.MaxSpriteRatio", 0.1f);		//OK
		QUALITY_CVAR("r.DOF.Recombine.Quality", 2);				//OK
		QUALITY_CVAR("r.DOF.Recombine.EnableBokehSettings", 1);	//OK
		QUALITY_CVAR("r.DOF.TemporalAAQuality", 1);				//OK
		QUALITY_CVAR("r.DOF.Kernel.MaxForegroundRadius", 0.025f);		//OK
		QUALITY_CVAR("r.DOF.Kernel.MaxBackgroundRadius", 0.025f);		//OK
		...
		// //质量设定
		QUALITY_CVAR("sg.ViewDistanceQuality", 4);
		QUALITY_CVAR("sg.AntiAliasingQuality", 4);
		QUALITY_CVAR("sg.ShadowQuality", 4);
		QUALITY_CVAR("sg.PostProcessQuality", 4);
		QUALITY_CVAR("sg.TextureQuality", 4);
		QUALITY_CVAR("sg.FoliageQuality", 4);
		QUALITY_CVAR("sg.ShadingQuality", 4);
		//
		// //Lumen设定
		QUALITY_CVAR("ShowFlag.LumenReuseShadowMaps", 0);		//OK
		QUALITY_CVAR("r.Lumen.ScreenProbeGather.HardwareRayTracing.NormalBias",1000000);	//OK
